...
class _ItemListScreenState extends State<ItemListScreen> {
  ...
  _loadSelectedItems() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    List<String>? storedItems = prefs.getStringList(widget.machineId);
    lastMachineId = prefs.getString('lastMachineId') ?? '';
    if (storedItems != null) {
      setState(() {
        selectedIds = storedItems;
        cartItemCount = storedItems.length;
  ...
  _saveSelectedItems() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setStringList(widget.machineId, selectedIds);
    await prefs.setString('lastMachineId', widget.machineId);
  }
  ...
  void onCartTapped() {
    if (cartItemCount > 0) {
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) =>
              CartPage(selectedIds: selectedIds, machineId: lastMachineId),
        ),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
          ...
          onPressed: () {
            if (cartItemCount > 0) {
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) => CartPage(
                      selectedIds: selectedIds, machineId: lastMachineId),
                ),
              );
            } else {
              ScaffoldMessenger.of(context).showSnackBar(
